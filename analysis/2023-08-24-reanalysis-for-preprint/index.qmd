---
title: "qPCR analysis of stability (degradation) experiment"
author: 
  - Michael McLaren
  - Daniel Rice
---
...
Note from Mike:
I copied this script from Dan's file in initial-analysis-37d, and modified to create an improved figure for the manuscript.
For that reason, it may be somewhat confusing, and we should revise it for the next manuscript version.
...
```{r}
data_dir <- here("data/37-day-qpcr")
...
data_raw <- list.files(
  ...
  list_rbind() %>%
  mutate(
    source_file = path_rel(source_file, data_dir)
  )
data_raw %>%
  head(n = 10) %>%
  kable()
...
```{r}
data_raw %>% pull(sample_name)
```


Currently we have the `sample_name` column, which contains three pieces of information: the experimental condition (e.g., "A", "B", ...), the timepoint, and the technical replicate number.
...
data_tidy <- data_raw |>
  ...
  mutate(
    ...
    # simplify source file by removing path up to data_path
    plate = str_extract(source_file, "3-7 [1-9]+"),
    plate_well = str_c(plate, well, sep = '_')
  )
kable(head(data_tidy, n = 10))
```

Let's make sure we have 24 blanks, 23 NTCs, 45 wastewater + phagemid and TBS + phagemid (3 qPCR replicates * 3 technical replicates * 5 timepoints), 63 bleach and wastewater (7 timepoints).
...
Again we see that the Blanks have lower than expected CT.
These are investigated below and shown to be artefacts of baseline subtraction; thus they can be ignored.
...
```{r}
standard_curve_file <- here("data", "T1_QS2_018_BC49 and T1_QS2_018_BC04.xls")
...
## Plot and summary stats for manuscript

There is a lot of variation around the overall trend, from different sources

- day-to-day variation
- variation among technical replicates
- variation among qPCR replicates

We want a plot that shows this variation, so that we can understand the limits of what can really be said here.
To do so, it is important to plot the individual replicates as separate datapoints.
I'll summarize the qPCR replicates on the plot by putting a point at the median (middle of the three qPCR replicates) and bars to the min and max qPCR replicates.
...
data_concentration |>
  ...
  ggplot(
    aes(timepoint, log10_concentration, color=treatment_group, 
...
```{r, eval = F}
ggsave('manuscript-figure.png', dpi = 300, width = 6, height = 4.5, units = 'in')
```

The variability suggests standard linear regression is not appropriate.
Instead, let's just show the data as is and avoid making quantitative estimates of half life.
